...
class Item {
    ...
    is_mouse_edge(mouseX, mouseY) {
        ...
        diagonal_dist = dist(mouseX, mouseY, this.body.position.x, this.body.position.y) - radius
...
function mouseMoved(){
    ...
    items.forEach((item)=>{
        if(
            item.is_mouse_edge(mouseX, mouseY)
        ){
            // 마우스 들어온 각도
            let input_angle = atan2(item.body.position.x - mouseX, item.body.position.y - mouseY)            
